...
var build = function(tables)
{
	...
	db.insertEntity = function(typeId, name)
	{
		...
		if(name != null)
		{
			var nameData = db.getOrInsertName(name);
			db.mapEntities(data.id, nameData.id);
		}

		return data;
	};

	db.insertEntityWithData = function(tableId, data, name)
	{
		var entity = db.insertEntity(tableId, name);
		data.id = entity.id;
		var result = db.insert(tableId, data);
		return result;
	};

	db.getName = function(name)
	{
		var result = nameTable({ name: name });
		if(result.count() > 1)
		{
	...
	db.insertName = function(name)
	{
		var result = db.insertEntityWithData(tableIds.name, { name: name });
		return result;
	};

	db.getOrInsertName = function(name)
	{
		var result = db.getName(name);
		if(!result)
		{
			result = db.insertName(name);
		}
...
var buildFromData = function(dbData)
{
	var tables = buildTables(dbData);
	var db = build(tables);
	return db;
...
autoBuild.buildFromData = buildFromData;
